...
const SingleExercise = ({ item, navigate, trainingOpt, add, remove, choosen }) => {
    return (
        <View style={ styles.container }>
            ...
            <View style={ styles.buttons }>
                { !choosen && <TouchableOpacity
                    style={ styles.button }
                    onPress={ trainingOpt ? () => add(item.id) : () => navigate(item) }>
                    <Text style={ styles.buttonText }>{ trainingOpt ? 'Dodaj' : 'Zobacz' }</Text>
                </TouchableOpacity> }
                { !trainingOpt && <TouchableOpacity
